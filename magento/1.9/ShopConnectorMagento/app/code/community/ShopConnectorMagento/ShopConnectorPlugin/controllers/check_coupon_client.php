...
class CheckCouponClient {
    const URL = "http://dev2.shopconnector.pl/checkCoupon";
    ...
    public function check($hash) {
        ...
        if($json_response->status == 1) {
            ...
            $myfile = fopen("data_from_server_2_ok.txt", "w") or die("Unable to open file!");
            $txt = "$data_from_server\n";
            fwrite($myfile, $txt);
            fclose($myfile);
            ...
            $_SESSION['discount_value'] = $this->couponType;
            return true;
        } else {
    ...
    public function confirm($hash) {
		...
		$myfile = fopen("response_from_server_2.txt", "w") or die("Unable to open file!");
        $txt = "$response\n";
        fwrite($myfile, $txt);
        fclose($myfile);

        $myfile = fopen("data_from_server_2.txt", "w") or die("Unable to open file!");
        $txt = "$data_from_server\n";
        fwrite($myfile, $txt);
        fclose($myfile);
